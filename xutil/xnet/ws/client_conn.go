...
var _ xnet.Conn = &clientConn{}

func newClientConn(id int64, conn *websocket.Conn, client *client) xnet.Conn {
	c := &clientConn{
		id:                id,
		conn:              conn,
		state:             int32(xnet.ConnOpened),
		client:            client,
...
// State 获取连接状态
func (c *clientConn) State() xnet.ConnState {
			...
			if err != nil {
				if !errors.Is(err, net.ErrClosed) {
					if _, ok := err.(*websocket.CloseError); !ok {
						xlog.Warn().Msgf("read message failed: %v", err)
					}
				}
				_ = c.forceClose()
				return
...
func (c *clientConn) doWrite(conn *websocket.Conn, r chWrite) bool {
	...
	if err := conn.WriteMessage(websocket.BinaryMessage, r.msg); err != nil {
		if !errors.Is(err, net.ErrClosed) {
			if _, ok := err.(*websocket.CloseError); !ok {
				xlog.Error().Msgf("write message error: %v", err)
			}
		}
	}
