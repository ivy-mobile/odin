...
func NewPacker(opts ...Option) *defaultPacker {
	...
	if o.routeBytes != 1 && o.routeBytes != 2 && o.routeBytes != 4 {
		xlog.Fatal().Msgf("the number of route bytes must be 1、2、4, and give %d", o.routeBytes)
	}

	if o.seqBytes != 0 && o.seqBytes != 1 && o.seqBytes != 2 && o.seqBytes != 4 {
		xlog.Fatal().Msgf("the number of seq bytes must be 0、1、2、4, and give %d", o.seqBytes)
	}

	if o.bufferBytes < 0 {
		xlog.Fatal().Msgf("the number of buffer bytes must be greater than or equal to 0, and give %d", o.bufferBytes)
	}
...
// PackBuffer 打包消息
func (p *defaultPacker) PackBuffer(message *Message) (xbuffer.Buffer, error) {
	if message.Route > int32(1<<(8*p.opts.routeBytes-1)-1) || message.Route < int32(-1<<(8*p.opts.routeBytes-1)) {
		return nil, ErrRouteOverflow
	}
